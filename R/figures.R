...
spc_tile_plot <- function(spca, column, fontface="bold", option="viridis", begin=0, end=1) (list(spc.genes = spca[, column, drop=T] %>% sort(dec=T) %>% subset(. != 0) %>% names) %>% with(
  data.frame(
  ...
  + geom_text(aes(x='', y=gene, label=gene), size=4, fontface=fontface, color='white')
