...
midgutClusteringParameterSweep_rann <- function(
  indrop, dim_reduc, dims, res_range,
  k.param = 20, prune.SNN = 1/k.param/0.75
) {
  no_clusters <- rep(NA, ncol(indrop))
  genes_of_interest <- c(
    "esg", "Dl", "N", "His3.3A", "Larp4B",
    "IA-2", "NPF", "pros",
    "MtnA", "Amy-p", "yin"
  )
  no_genes <- matrix(
    ...
    ncol = 2,
    dimnames = list(genes_of_interest, c("intercept", "clusterEB"))
  )
  indrop[["dimreduc"]] <- dim_reduc
  tibble(
    dims = dims,
    snn = sapply(
      dims,
      \(d) FindNeighbors(
        indrop, red="dimreduc", dims=1:d, verb=FALSE,
        nn.method = "rann",
    ...
    rowwise %>%
    mutate(
      res = pickClusteringResolution(indrop, snn, res_range = res_range) %>%
        binsearchChooseX %>%
        `*`(0.01),
      random.seed = 0,
      clusters =
        if (!is.na(res))
          clusterAndLabelIscEb(indrop, snn, res, random.seed) %>%
          list
        else list(no_clusters),
      log2FoldChange =
        if (!is.na(clusters[1]))
          indrop %>%
            build_rough_glm(
...
midgutClusteringParameterSweep_annoy <- function(
  indrop, dim_reduc, dims, res_range,
